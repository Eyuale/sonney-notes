...
if (!REGION || !BUCKET) {
  ...
  console.warn("[lib/s3] Missing AWS_REGION_NAME or S3_BUCKET_NAME env var");
}
...
export async function presignPutUrl(params: {
  ...
  if (!REGION || !BUCKET) throw new Error("S3 not configured: set AWS_REGION_NAME and S3_BUCKET_NAME");
